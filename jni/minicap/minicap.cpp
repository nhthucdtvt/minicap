...
public:
    FrameWaiter()
        : mPendingFrames(0),
          mTimeout(std::chrono::milliseconds(50)),
          mStopped(false)
    {
    }
...
int main(int argc, char *argv[])
{
    ...
    while (!gWaiter.isStopped() && (fd = server.accept()) > 0)
    {
        ...
        MCINFO("after client connection");
        int pending, err;
        bool isSend = false;
        char buffer[256];
        ...
        while (!gWaiter.isStopped())
        {
            ...
            // MCINFO("wait select timeout");
            int activity = select(fd + 1, &fds, nullptr, nullptr, &timeout);

            if (activity > 0)
            {
                if (FD_ISSET(fd, &fds))
                {
                    // We have incoming data on the socket
            ...
            if ((pending = gWaiter.waitForFrame()) > 0)
            {
                ...
                if (skipFrames && pending > 1)
                {
                    ...
                    while (--pending >= 1)
                    {
                        ...
                        MCINFO("While pending %d", pending);

                        minicap->releaseConsumedFrame(&frame);
                    }
                }

                err = minicap->consumePendingFrame(&frame);
                MCINFO("consumePendingFrame");
                if (err != 0)
                {
                    if (err == -EINTR)
                    {
                ...
                // Encode the frame.
                bool encodeErr = encoder.encode(&frame, quality);
                MCINFO("endcode");
                if (!encodeErr)
                {
                    MCERROR("Unable to encode frame");
